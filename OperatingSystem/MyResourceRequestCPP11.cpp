...
void ResourceRequest(SystemInfo& info, RequestResps& result)
{
    for (int j = 0; j < p; ++j) {
        ...
        cout << "\n\tAllocation[" << i << "] = Allocation[" << i << "] + Request[" << i << "]=" << info.Allocation[i];
        cout << "\n\tNeed[" << i << "] = Need[" << i << "] - Request[" << i << "]=" << info.Need[i];
        if (Banker(info)) {
...
#define taskname "MyResourceRequest"
